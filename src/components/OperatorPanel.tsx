...
export default function OperatorPanel() {
  ...
  const playAnnouncement = (item: Announcement, isCustom: boolean = false) => {
    ...
    if (playbackOutput === 'local' || playbackOutput === 'both') {
      const audioSrc = isCustom 
        ? (customFiles.find(f => f.name === item.audioUrl)?.url || item.audioUrl) 
        : item.audioUrl;
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col md:flex-row pb-12" id="operator_viewport_container">
      ...
      <div className="w-full md:w-80 bg-white text-slate-800 p-6 flex flex-col justify-between border-r border-slate-250 shadow-sm" id="operator_sidebar">
        <div>
          ...
          <div className="space-y-2 mb-5">
            <label className="text-xs font-bold uppercase tracking-wider text-slate-450 block font-sans">
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-4" id="live_broadcasting_strip">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-550 mt-0.5">
                      ...
                      <div 
                        ...
                        className={`p-4 rounded-xl border text-left transition-all hover:translate-y-[-1px] hover:shadow-md cursor-pointer group flex flex-col justify-between min-h-[140px] ${
                          isSuccess
                            ...
                            : 'bg-white border-rose-100 hover:border-rose-500 hover:bg-rose-50 bg-rose-50/10 border-l-4 border-l-rose-500'
                        }`}
                      >
                        <div className="space-y-1">
                          <div className="flex items-center justify-between gap-1.5">
                            <span className={`text-[9px] uppercase tracking-wider font-bold ${
                              isSuccess ? 'text-emerald-600' : isWarning ? 'text-amber-600' : 'text-rose-600'
                            }`}>
                              {isSuccess ? 'GERAL' : isWarning ? 'SEGURANÇA & LOGÍSTICA' : 'URGENTE'}
                            </span>
                        ...
                        <div className="flex items-center justify-between border-t border-slate-100 pt-2.5 mt-2">
                          <button
                            ...
                            className="text-slate-450 hover:text-slate-700 hover:bg-slate-100 p-1.5 rounded-lg transition-colors flex items-center gap-1 text-[11px] font-semibold"
                          ...
                          <span className={`text-xs px-2.5 py-1 rounded-md font-bold transition-all text-white flex items-center gap-1 ${
                            ...
                            isWarning ? 'bg-amber-500 group-hover:bg-amber-600' : 'bg-rose-600 group-hover:bg-rose-700'
                          }`}>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-4" id="custom_tts_microphone_widget">
              <div>
                ...
                <p className="text-xs text-slate-450 font-medium">Insira um aviso customizado de viva-voz para ser proferido em lote</p>
              </div>
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-4" id="announcement_queue_card">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-base font-bold text-slate-800 font-sans flex items-center gap-2">
                    <Layers className="h-5 w-5 text-emerald-655" /> Fila de Sonorização
                  </h3>
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          {item.status === 'pending' && (
                            <button
                              ...
                              className="text-emerald-700 hover:text-emerald-655 bg-emerald-50 hover:bg-emerald-100 p-1 rounded-md"
                              title="Tocar agora"
                            >
