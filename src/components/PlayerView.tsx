...
export default function PlayerView() {
  ...
  const triggerReceiverPlay = async (
    ...
    const onAudioEnded = () => {
      // Sound finished playing, but we let the visual countdown timer complete the visual presentation on the wall TV
    };
  ...
  return (
    <div className="min-h-full flex-1 w-full bg-slate-50 text-slate-800 flex flex-col p-4 sm:p-6 md:p-8" id="player_terminal_viewport">
      ...
      {/* Autoplay Warning Banner if locked */}
      {!isAudioUnlocked && (
        <div className="mt-4 bg-amber-50 border border-amber-200 rounded-xl p-3 flex flex-col sm:flex-row items-center justify-between gap-3 shadow-xs animate-subtle-bounce">
          <div className="flex items-center gap-2.5 text-amber-850">
            <Volume2 className="h-5 w-5 text-amber-600 shrink-0 animate-pulse" />
            <div className="text-left animate-pulse">
              <p className="text-xs font-bold font-sans">Áudio do Navegador Bloqueado nesta TV</p>
              <p className="text-[11px] text-slate-500 leading-normal font-sans">
                Para ouvir os avisos sonoros de forma sincronizada neste computador, clique no botão ao lado apenas uma vez.
              </p>
            </div>
          </div>
          <button
            onClick={unlockAudio}
            className="w-full sm:w-auto px-4 py-2 bg-orange-600 hover:bg-orange-500 active:scale-95 text-white font-sans text-xs font-bold rounded-lg shadow-sm flex items-center justify-center gap-1.5 transition-all cursor-pointer select-none"
          >
            <Volume2 className="h-3.5 w-3.5" /> Ativar Som do Receptor
          </button>
        </div>
      )}
